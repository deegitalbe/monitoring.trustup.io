                ...
                @foreach($domain_ping_batches as $domain_ping_batch)
                    <tr>
                        <td class="pr-6 pl-2 py-0.5 flex space-x-2 items-center h-full">
                            <i class="fas fa-circle text-xs {{ $domain_ping_batch->finished_at && !$domain_ping_batch->failed ? 'text-green-400' : (!$domain_ping_batch->failed ? 'text-blue-300 animate-ping' : 'text-red-400' )}}"></i>
